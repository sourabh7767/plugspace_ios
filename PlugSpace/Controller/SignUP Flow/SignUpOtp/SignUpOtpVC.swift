...
class SignUpOtpVC: BaseVC {
    ...
    private func setupUI() {
        ...
        setAfter { [self] in
            ...
            for view in txtView {
                view.cornerRadius = 8
                view.setShadow(4.0, 0.0, 2, AppColor.Orange, 0.2)
            }
...
//                combinedArr = viewModel.otp[0].otpcode ?? ""
    ...
    @IBAction private func onBtnVerifyCode(_ sender: UIButton) {
        ...
        viewModel.verifyOTP { [self] (isSuccess) in
            ...
            if isSuccess {
                SignUPVM.shared.phone = txtNumber
                SignUPVM.shared.ccode = ccode
                SignUPVM.shared.isApple = "0"
                SignUPVM.shared.appleId = "0"
                SignUPVM.shared.isInsta = "0"
                SignUPVM.shared.instaId = "0"
                viewModel.isLogin == "1" && viewModel.data != nil ? alertWith(message: viewModel.errorMessage) : pushVC(GenderVC.self,animated: false)
            }else {
                alertWith(message: viewModel.errorMessage)
            }
        }
...
extension SignUpOtpVC: UITextFieldDelegate, MyTextFieldDelegate {
