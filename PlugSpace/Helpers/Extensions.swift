...
func getThumnailImage(videoURL:String) -> UIImage {
...
extension Sequence where Iterator.Element == UIView {
    ...
    func setRoundBorder(_ width: CGFloat, _ color: UIColor) {
        self.forEach { (v) in
            v.setRoundBorder(width, color)
        }
    }
    
    func setBorder(_ width: CGFloat, _ color: UIColor, _ cornerRadius: CGFloat) {
        self.forEach { (v) in
...
extension UIViewController {
    ...
    func backNormalVC(animated: Bool) {
    ...
    func alertWith(_ title: String? = AppName, message: String?, type: UIAlertController.Style = UIAlertController.Style.alert, cancelTitle:String? = "OK",  othersTitle: [String] = [], sheetSourceView: UIView? = nil, cancelTap:(()->())? = nil, othersTap: ((_ index: Int, _ title: String)->())? = nil) {
        ...
        if type == .actionSheet, sheetSourceView != nil, UIDevice.current.userInterfaceIdiom == UIUserInterfaceIdiom.pad {
...
extension UIImageView {
    ...
    public func setShadowImage(color: UIColor, opacity: Float, radius: CGFloat) {
...
extension UIView {
    ...
    func Set_border_white( _ borderColor: String = "ffffff",_ Corners: UIRectCorner = [.topRight, .bottomLeft])
    {
...
extension String {
    ...
    func isValidPhoneNumber() -> Bool {
        ...
        return self == filtered && self.count > 5 && self.count < 12
    }
...
extension Date {
    ...
    public func totalYear(ofComponent comp: Calendar.Component, fromDate date: Date) -> Int {
    ...
    func lessthenYears() -> Int {
        let interval = Calendar.current.dateComponents([.year], from: self, to: Date()).year!
        if interval > 0 {
    ...
    public func getTimeAgoSinceNow() -> String {
        let interval = Calendar.current.dateComponents([.year], from: self, to: Date()).year!
        if interval > 0 {
            return interval == 1 ? "\(interval)" + " year old" : "\(interval)" + " years old"
        }
        return "Now"
    }
}
...
extension UITableView {
    ...
    func EmptyMessage(message:String) {
        ...
        messageLabel.font = UIFont(name: AppFont.bold, size: 20)
...
extension UICollectionView {
    ...
    func EmptyMessage(message:String) {
        ...
        messageLabel.textAlignment = .center;
        messageLabel.font = UIFont(name: AppFont.bold, size: 17)
        messageLabel.sizeToFit()
...
extension CGFloat {
    ...
    public func roundNumber(value:CGFloat) -> String {
...
extension UIViewController {
    ...
    func showToast(message : String,y:CGFloat, font: UIFont) {
        
        let x = (UIScreen.Width - (message.widthOfString(usingFont: font) + 20)) / 2
        let toastLabel = UILabel(frame: CGRect(x: x, y: y, width: message.widthOfString(usingFont: font) + 20, height: 40))
    ...
    func openPhotoSelectionOption(for imagePicker:UIImagePickerController, type : MediaType) {
        ...
        actionSheet.addAction(galaryAction)
